...
<xsl:text>&#xa;</xsl:text>
    ...
    <h2><xsl:value-of select="subproject[@id='landforms']/name"/></h2>
    <p>Responsible: <xsl:value-of select="subproject[@id='landforms']/responsible"/></p>

    <xsl:copy-of select="subproject[@id='landforms']/description"/>
    </xsl:for-each>
