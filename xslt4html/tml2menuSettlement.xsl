...
<xsl:text>&#xa;</xsl:text>
    ...
    <h2><xsl:value-of select="subproject[@id='settlements']/name"/></h2>
    <p>Responsible: <xsl:value-of select="subproject[@id='settlements']/responsible"/></p>    

    <xsl:copy-of select="subproject[@id='settlements']/description"/>
    </xsl:for-each>
    ...
    <xsl:for-each select="ent">
        ...
        <xsl:choose>
		<xsl:when test="types/type='settlement'">
		<li>
                  <xsl:element name="a">
