...
<xsl:text>&#xa;</xsl:text>
    ...
    <h2><xsl:value-of select="subproject[@id='bells']/name"/></h2>
    <div>Responsible: <xsl:value-of select="subproject[@id='bells']/responsible"/></div>

    <xsl:copy-of select="subproject[@id='bells']/description"/>
    </xsl:for-each>
